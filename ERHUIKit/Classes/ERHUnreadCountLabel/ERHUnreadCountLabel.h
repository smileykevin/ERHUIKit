...
//     请勿直接将 ERHUnreadCountLabel 实例添加到目标父视图，
//     此时可以在目标父视图相同位置添加一个坐标与其完全一致的 containerView 作为父视图，
//     并将 ERHUnreadCountLabel 实例作为子视图添加到此 containerView 上
//  3. ERHUnreadCountLabel 实例的显示/隐藏，仅由 unreadCount 即可控制，
...
typedef NS_ENUM(NSInteger, ERHUnreadCountLabelPosition) {
    ERHUnreadCountLabelPositionTopRight = 0,        // 位于父视图的右上角，通常用于圆形 Icon 右上角展示，目前默认其宽高相等
    ERHUnreadCountLabelPositionRight    = 1         // 位于父视图的最右侧，通常用于 Cell 最右侧展示
};

NS_ASSUME_NONNULL_BEGIN

@interface ERHUnreadCountLabel : UILabel
